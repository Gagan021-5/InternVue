      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-[minmax(220px,1.3fr)_minmax(260px,1.4fr)_130px_minmax(160px,auto)_auto] md:items-end">
        ...
        <div className="relative space-y-1.5">
          <label className="flex flex-col space-y-1.5">
            <span className="font-mono text-xs font-semibold uppercase tracking-wider text-blue-600">Where</span>
            <input
              type="text"
              ...
              className="input-premium w-full px-4 py-2.5 text-sm placeholder:text-slate-400 disabled:opacity-60 md:text-base"
            />
          </label>

          {suggestOpen ? (
            <div className="section-shell absolute left-0 right-0 top-[calc(100%+8px)] z-50 max-h-64 overflow-auto p-1.5">
              {suggestions.map((suggestion) => (
                <button
                  key={suggestion.id}
                  type="button"
                  onClick={() => onSelectSuggestion(suggestion)}
                  className="block w-full rounded-xl px-4 py-2.5 text-left text-sm text-soft transition-colors hover:bg-slate-100"
                >
                  {suggestion.label}
                </button>
              ))}
          ...
          {suggestLoading ? <p className="mt-1 px-2 text-xs text-muted">Looking up cities...</p> : null}
        </div>

        <label className="space-y-1.5">
          <span className="font-mono text-xs font-semibold uppercase tracking-wider text-blue-600">Radius</span>
          <select
            value={radius}
            onChange={(event) => setRadius(event.target.value)}
            className="input-premium h-[46px] w-full px-3 py-2.5 text-sm md:text-base"
          >
        ...
        <button
          ...
          className="btn-secondary flex h-[46px] items-center justify-center gap-2 px-4 text-sm md:text-base"
          disabled={geoLoading || radius === "remote"}
          title={radius === "remote" ? "Location is not needed for remote search" : "Detect my location"}
        >
          <span>GPS</span> {geoLoading ? "Detecting..." : "Detect Location"}
        </button>

        <button type="submit" className="btn-primary h-[46px] px-8 text-sm font-bold tracking-wide md:text-base">
          Search
        </button>
      </div>
    </form>
